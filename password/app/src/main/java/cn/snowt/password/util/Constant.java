...
public class Constant {
    ...
    public static final String STRING_HELP = "" +
            ...
            "\n二、推荐正确用法\n" +
            "本软件虽说是密码管理器，但还是不建议直接将密码存储在本软件。推荐的用法是：账号直接填写在本软件，密码则是填写密码提示。例如：【我的google账号为：10086，账号直接存10086；我的真正密码为@google20210804,但我绝不会将密码存储在本软件，我只会存个提示@谁210804，@为特殊字符，谁表示的是什么账号，210804表示的是账号注册日期】。建议使用者举一反三。通常，我们需要注册一些无关紧要的账号，比如去某个论坛下载破解软件，但该论坛需要登录回复帖子才可以下载，这种账号我们大概率一年也用不到几次，这时，就建议使用密码生成功能，生成一个密码直接存储在本软件，而这个账号就算没了也无关紧要。\n" +
            "\n三、登录有关\n" +
            ...
            "2023-03-18作者发现安卓对‘Android/data目录’的读写做了限制，部分机型不能将data目录中的数据复制走，甚至不能访问。但本软件的备份数据、导出数据都在data目录下，安卓更新导致权限越收越紧，很烦。本来我是不想让这个软件申请任何权限的，但已有用户反馈访问不到data目录了，那我只能申请权限将文件存储在外部存储中。\n" +
            "\n五、权限使用及外部存储的说明\n" +
            "敏感权限目前只使用了”读写外部存储“，用于输出备份文件、数据导出，按理来说，你不授权也能正常使用本软件。外部存储的目录为【外部存储\\Hibara\\Password】，卸载软件时不会删除此文件夹，需要你手动删除。\n" +
            "\n六、密码生成相关\n" +
            ...
            "\n七、写在最后\n" +
            "1.本软件绝不会盗取你的数据，下面有网址有开源代码可查。因此，你在使用本软件时产生的任何形式的损失都与本作者无关！\n" +
    ...
    public static final String EXTERNAL_STORAGE_LOCATION = "/Hibara/Password/";
}
